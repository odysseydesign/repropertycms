...
// Load Stripe secret key and webhook secret from config/environment variables
$stripeSecretKey = getenv('STRIPE_KEY') ?: (defined('config') && function_exists('config') ? config('services.stripe.secret') : null);
$endpoint_secret = getenv('STRIPE_WEBHOOK_SECRET') ?: (defined('config') && function_exists('config') ? config('services.stripe.webhook_secret') : null);

if (!$stripeSecretKey || !$endpoint_secret) {
...
try {
    $event = \Stripe\Webhook::constructEvent(
        ...
        // Here store seprated data in seprated variables
        $payment_date = date('Y-m-d H:i:s', $data['created']),
        $payment_id = $data['id'],
        $payment_amount = $data['data']['object']['amount'] / 100,
...
switch ($event->type) {
    ...
    case 'payment_intent.succeeded':
        $paymentIntent = $event->data->object;
        $sql = "INSERT INTO payments (agent_id, plan_id, payment_date, payment_id, amount , currency , status )
    VALUES ('114','3','$payment_date', '$payment_id','$payment_amount','$payment_currency','paid')";
        $conn->query($sql);

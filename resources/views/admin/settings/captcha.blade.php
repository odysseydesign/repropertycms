...
@section('content')
    ...
    <div class="card p-6 bg-white border rounded-xl shadow-sm" style="max-width:680px;">
        ...
        <div class="alert mb-4" style="background:#eff6ff;border:1px solid #bfdbfe;color:#1e40af;font-size:13px;padding:12px 16px;border-radius:8px;">
            <p class="font-medium mb-1"><i class="fas fa-info-circle mr-1"></i> How to get reCAPTCHA keys:</p>
            <ol class="mb-0" style="padding-left:1.25rem;font-size:12px;">
                <li>Go to <span style="font-family:monospace;">google.com/recaptcha/admin</span></li>
        ...
        <div x-data="{
            ...
            <form method="POST" action="{{ route('admin.settings.captcha.save') }}" class="space-y-4">
                @csrf

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Site Key</label>
                    <input type="text" name="site_key" x-model="siteKey"
                        placeholder="6L..."
                        class="form-control" style="font-family:monospace;">
                    <p class="text-xs text-gray-400 mt-1">Used in the frontend. Safe to make public.</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Secret Key</label>
                    <input type="password" name="secret_key" x-model="secretKey"
                        placeholder="{{ $current['secret_set'] ? '••••••••  (leave blank to keep current)' : '6L...' }}"
                        class="form-control" style="font-family:monospace;">
                ...
                <div class="d-flex align-items-center justify-content-between pt-3" style="margin-top:1rem;padding-top:1rem;border-top:1px solid #e5e7eb;">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <button type="button" @click="runTest()" :disabled="testing"
                            ...
                            <span x-show="!testing">Validate Keys</span>
                            <span x-show="testing">Checking...</span>
                        </button>
